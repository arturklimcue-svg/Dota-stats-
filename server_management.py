...
class PatchViewPage(discord.ui.View):
    ...
    def __init__(self, patches: list, page: int):
        super().__init__(timeout=None)
        self.patches = patches
        self.page = page
        total = len(patches)
        self.children[0].disabled = page <= 0
        self.children[1].disabled = page >= total - 1

    @discord.ui.button(label="⬅️ Предыдущий", style=discord.ButtonStyle.secondary,
                        custom_id="patch:page:prev")
    async def prev_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        new_view = PatchViewPage(self.patches, max(0, self.page - 1))
        await interaction.edit_original_response(
            embed=new_view._build_patch_embed(self.patches[self.page - 1]),
            view=new_view)

    @discord.ui.button(label="Следующий ➡️", style=discord.ButtonStyle.secondary,
                        custom_id="patch:page:next")
    async def next_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer(ephemeral=True)
        new_view = PatchViewPage(self.patches, min(len(self.patches) - 1, self.page + 1))
        await interaction.edit_original_response(
            embed=new_view._build_patch_embed(self.patches[self.page + 1]),
            view=new_view)
    ...
    @staticmethod
    def _build_patch_embed(p: dict) -> discord.Embed:
        name = p.get("name", "?")
        date_ts = p.get("release_date", 0)
        date_str = datetime.utcfromtimestamp(date_ts).strftime("%d.%m.%Y") if date_ts else "?"
        url = p.get("url", "")
        desc = f"**Дата:** {date_str}"
        if url:
            desc += f"\n**Полный список изменений:** [dota2.com]({url})"
        embed = discord.Embed(title=f"📊 {name}", description=desc, color=0x8B4513)
...
class PatchListView(discord.ui.View):
    ...
    async def list_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.followup.send(embed=embed, view=PatchListViewWithNumbers(patches), ephemeral=True)
    ...
    async def build_embed(self) -> discord.Embed:
        ...
        try:
            ...
            pass
        PatchListView._last_patches = patches
        if not patches:
            return discord.Embed(
...
class PatchListViewWithNumbers(discord.ui.View):
    """Вид со списком патчей и кнопками для выбора."""
    def __init__(self, patches: list):
        super().__init__(timeout=None)
        self.patches = patches
        for i, p in enumerate(patches):
            name = p.get("name", f"Патч {i+1}")
            btn = discord.ui.Button(label=name, style=discord.ButtonStyle.primary,
                                    custom_id=f"patch:view:{i}")
            btn.callback = lambda inter, idx=i: self._show_patch(inter, idx)
            self.add_item(btn)

    async def _show_patch(self, interaction: discord.Interaction, index: int):
        await interaction.response.defer(ephemeral=True)
        p = self.patches[index]
        embed = PatchViewPage._build_patch_embed(p)
        view = PatchViewPage(self.patches, index)
...
class ServerManagement(commands.Cog):
    ...
    async def cog_load(self):
        ...
        self.bot.add_view(PatchViewPage([], 0))
    ...
    async def server_setup(self, ctx: commands.Context):
        ...
        vr_combined_embed = discord.Embed(
            ...
            color=0x2B2D31)
        vr_combined_view = discord.ui.View(timeout=None)
        ...
        if rules_ch:
            rules_combined_embed = discord.Embed(
                ...
                color=0x8B4513)
            rules_combined_view = discord.ui.View(timeout=None)
        ...
        if strat_ch:
            strat_embed = discord.Embed(
                ...
                color=0x8B4513)
            strat_view = discord.ui.View(timeout=None)
        ...
        if events_ch:
            events_embed = discord.Embed(
                ...
                color=0x8B4513)
            events_view = discord.ui.View(timeout=None)
